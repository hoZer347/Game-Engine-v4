...
namespace loom
{
	...
	constexpr void Load(auto& mesh) noexcept
	{ return _Att<decltype(mesh)>::Load(mesh); };

	constexpr void Free(auto& mesh) noexcept
	{ return _Att<decltype(mesh)>::Free(mesh); };

	template <typename T>
	constexpr T& Get(auto& mesh) noexcept
	{ return Att<decltype(mesh), T>::Get(mesh); };
};
